...
const pkg = require('../../_nodedash/package.json')
const version = pkg.version

const _path = path.join(basePath, '_nodedash/nodedash.js')
...
const config = {
	base: {
		path: _path,
		title: `<a href="https://github.com/flavioespinoza/flodash">flodash</a> <span>v${version}</span>`,
		toc: 'categories',
		pkg: pkg,
		url: npmLink()
	},
...
function npmLink() {
	return (
		'<% if (name == "templateSettings" || !/^(?:methods|properties|seq)$/i.test(category)) { print("https://github.com/flavioespinoza/flodash/blob/master/methods/" + name + ".js' +
		'");} %>'
	)
}
